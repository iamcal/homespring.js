...
HS.Program.prototype.create_node = function(uid, name, parent){
	...
	if (HS.NodeDefs[lname]){
		//console.log('special node: '+lname);
...
HS.Node.prototype.hasMatureDownstreamSalmon = function(){
	for (var i=0; i<this.salmon.length; i++){
		if (this.salmon[i].age == HS.const.MATURE && this.salmon[i].direction == HS.const.DOWNSTREAM) return true;
	}
	return false;
}

HS.Node.prototype.hasMatureUpstreamSalmon = function(){
	for (var i=0; i<this.salmon.length; i++){
		if (this.salmon[i].age == HS.const.MATURE && this.salmon[i].direction == HS.const.UPSTREAM) return true;
	}
	return false;
}
...
HS.Salmon.prototype.toString = function(){
	var dir = this.direction == HS.const.DOWNSTREAM ? 'downstream' : (this.direction == HS.const.UPSTREAM ? 'upstream' : '?');
	var age = this.age == HS.const.YOUNG ? 'young' : (this.age == HS.const.MATURE ? 'mature' : '?');
	return this.uid+":"+this.name+'('+dir+'/'+age+')';
}
...
HS.Salmon.prototype.dumpState = function(){
...
HS.Program.prototype.run = function(amax){
	...
	// TODO: deal with the empty program here
...
HS.Program.prototype.dumpState = function(){
	...
	for (var i=0; i<self.nodes.length; i++){
		...
		var salmon = [];
		for (var j=0; j<n.salmon.length; j++){
			salmon.push(n.salmon[j].dumpState());
		}
		salmon = '['+salmon.join(',')+']';
