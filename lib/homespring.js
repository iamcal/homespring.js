...
HS.Program.prototype.emitEvent = function(type, data){
	data.type = type;
...
HS.Node.prototype.fishEnters = function(fish, fromNode){
	var self = this;

	if (fromNode){
		self.program.emitEvent('move', { salmon: fish.uid, from: fromNode.uid, to: self.uid });
	}
...
HS.Salmon.prototype.spawn = function(node){
	...
	self.program.emitEvent('spawn', { salmon: self.uid, child: ns.uid, node: node.uid });
	...
	for (var i=0; i<len; i++){
		if (chars[i] == ' '){
			...
			if (chars[i+1] == ' '){
				...
				if (self.options.strictMode){
					throw new HS.Exception('Illegal program - period followed by unexpected character: '+chars[i+1]);
				}

				// invalid production. push the current token onto the list,
				// followed by a blank token for the period.
	...
	self.preOrder(function(n){
		n.filterSalmon(function(s, remain){
			...
			if (!n.parent){
				...
				self.emitEvent('output', { salmon: s.uid, node: n.uid, name: s.name });
...
HS.Program.prototype.inputTick = function(){
	...
	if (self.input != null && self.input != undefined){
		...
		self.emitEvent('create', { salmon: s.uid, node: self.root.uid, source: 'input' });
...
HS.Program.prototype.dumpState = function(){
	...
	for (var i=0; i<self.nodes.length; i++){
		...
		console.log(pad+self.nodes[i].toString()+salmon);
	}
}
	...
	onMiscTick : function(){
		...
		this.filterSalmon(function(s, remain){
			if (s.age == HS.const.MATURE){
				s.dead = true;
				self.program.emitEvent('die', { salmon: s.uid, node: self.uid, cause: 'bear' });
			}else{
				remain.push(s);
			}
		});
	...
	onMiscTick : function(){
		...
		this.filterSalmon(function(s, remain){
			if (s.age == HS.const.YOUNG){
				s.dead = true;
				self.program.emitEvent('die', { salmon: s.uid, node: self.uid, cause: 'bird' });
			}else{
				remain.push(s);
			}
		});
...
HS.Nodes.hatchery = {
	...
	fishHatchTick : function(){
		if (this.isPowered() && !this.is_destroyed){
			var s = new HS.Salmon(this.program, "homeless", HS.const.YOUNG, HS.const.UPSTREAM, 1);
			this.born.unshift(s);
			this.program.emitEvent('create', { salmon: s.uid, node: this.uid, source: 'hatchery' });
		}
	}
};
