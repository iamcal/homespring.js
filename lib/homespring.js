...
HS.Node.prototype.hasMatureDownstreamSalmon = function(){
	var n = this.getDownstreamNodes();
	for (var i=0; i<n.length; i++)
		if (n[i].hasMatureSalmon())
			return true;
	return false;
}

HS.Node.prototype.getDownstreamNodes = function(){
	var list = [];
	var p = this.parent;
	while (p){
		list.push(p);
...
HS.Node.prototype.getUpstreamNodes = function(){
	var list = [];
	for (var i=0; i<this.kids.length; i++){
		list.push(this.kids[i]);
		list = list.concat(this.kids[i].getUpstreamNodes());
	}
	return list;
}
