...
HS.Program = function(source, debugMode){
	...
	self.preOrderList = [];
	self.preOrderPopulate(self.root);

	self.postOrderList = [];
	self.postOrderPopulate(self.root);
...
HS.Program.prototype.dumpState = function(){
	...
	for (var i=0; i<self.nodes.length; i++){
		...
		var salmon = [];
		for (var j=0; j<self.nodes[i].salmon.length; j++){
			salmon.push(self.nodes[i].salmon[j].dumpState());
		}
		salmon = '['+salmon.join(',')+']';

		console.log(("  ".repeat(depth))+self.nodes[i].safeName()+salmon);
	}
}
