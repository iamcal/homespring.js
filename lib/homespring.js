...
//	}
	...
	self.filterSalmon(function(s, remain){
		self.born.unshift(s.spawn(self));
		remain.push(s);
	});
...
HS.Nodes.clone = {
	onMiscTick : function(){
		...
		for (var i=0; i<n; i++){
			this.salmon.push(new HS.Salmon(this.program, this.salmon[i].name, HS.const.YOUNG, HS.const.DOWNSTREAM));
		}
	}
};
...
HS.Nodes.hatchery = {
	...
	fishHatchTick : function(){
		if (this.isPowered() && !this.is_destroyed){
			var s = new HS.Salmon(this, "homeless", HS.const.YOUNG, HS.const.UPSTREAM);
			this.born.unshift(s);
		}
	}
};
...
HS.Nodes.reverse_down = {
	onMiscTick : function(){
		var self = this;
		this.filterSalmon(function(s, remain){
			if (s.direction == HS.const.DOWNSTREAM && s.childFrom == 1 && self.kids.length > 1){
	...
	onMiscTick : function(){
		...
		this.filterSalmon(function(s, remain){
			...
			for (var i=0; i<chars.length; i++){
				self.born.push(new HS.Salmon(self.program, chars[i], s.age, s.direction));
			}
		});
