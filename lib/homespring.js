...
HS.Salmon = function(name, age, direction){
	var self = this;
	self.name = name;
	self.age = age;
...
HS.Program.prototype.fishTickHatch = function(){
	...
	self.preOrder(function(n){
		...
		if (n.lname == 'hatchery' && n.isPowered()){
			var s = new HS.Salmon("homeless", HS.const.MATURE, HS.const.UPSTREAM);
...
HS.Nodes.evaporates = {
	blocksWater : function(){ return this.is_powered; },
	blocksSnowmelt : function(){ return this.is_powered; }
};
...
HS.Nodes.shallows = {
	onFishEnters : function(fish){ fish.ready = false; }
};
